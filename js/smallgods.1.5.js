...
Tile.prototype.draw = function()
{
	...
	if(this.curResource > 0) {
		gDrawingContext.fillStyle = "rgba(0, 0, 255, "+Math.abs(this.curResource)/kResourceRange+")";
		gDrawingContext.fillRect(this.xPos, this.yPos, kTileSize, kTileSize);
	}
	if(this.curResource < 0) {
		gDrawingContext.fillStyle = "rgba(102, 51, 0, "+Math.abs(this.curResource)/kResourceRange+")";
		gDrawingContext.fillRect(this.xPos, this.yPos, kTileSize, kTileSize);
	}
}
...
Tile.prototype.update = function()
{
	...
	//World becomes either a desert or an oasis, there needs to be some kind of balancing mechanism, i.e. limit on the top and bottom of the current resources of a tile.
	var newResource = this.surResource / kResourceRange;
	if ((this.curResource + newResource < 0) && (this.curResource + newResource > kResourceRange*-1)) {
		this.curResource += newResource;
	}
	else if ((this.curResource + newResource < 0) && (this.curResource + newResource < kResourceRange)) {
		this.curResource = kResourceRange*-1;
	}
	if ((this.curResource + newResource > 0) && (this.curResource + newResource < kResourceRange)) {
		this.curResource += newResource;
	}
	else if ((this.curResource + newResource > 0) && (this.curResource + newResource > kResourceRange)) {
		this.curResource = kResourceRange;
	}
